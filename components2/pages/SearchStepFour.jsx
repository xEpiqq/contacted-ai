...
// CSS styles for table scrollbar and animations
...
function SearchStepFour({ 
  ...
  const onPageChange = (page) => {
    // This would actually call an API to fetch new results
  ...
  return (
    <>
      <motion.div
        key="search-step-four"
        initial={{ opacity: 0, x: 100 }}
        animate={{ opacity: 1, x: 0 }}
        exit={{ opacity: 0, x: -100 }}
        transition={{ type: "spring", stiffness: 300, damping: 30 }}
        className="w-full max-w-[690px] mx-auto"
      >
        {/* Results header */}
      ...
      {/* Inject table styles */}
      <style jsx global>{tableScrollbarStyles}</style>
    </>
  );
}
