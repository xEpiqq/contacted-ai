 ...
 *   we do NOT award the 20k tokens yet (so tokens_total = 0).
 * - Once the trial ends and subscription goes to "active", we award 20k tokens.
 */
export async function syncStripeDataToKV(stripeCustomerId, event = null) {
  try {
    ...
    if (priceId && PLAN_INFO[priceId]?.isRecurring) {
      ...
      // set tokens_total=100 (they only get the 20k after trial ends).
      if (
